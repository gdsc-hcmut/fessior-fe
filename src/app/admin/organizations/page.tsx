...
export default function Organizations() {
  ...
  const handleOrganizationDelete = useCallback(async (id: string) => {
    try {
      await organizationService.deleteOrganization(id);
      setOrganizations((prevOrganizations) => prevOrganizations?.filter((org) => org._id !== id) ?? null);
  ...
  const handleOrganizationUpdate = useCallback(async (organization: Organization) => {
    try {
      await organizationService.updateOrganization(organization);
      setOrganizations(
        (prevOrganizations) =>
          prevOrganizations?.map((org) => (org._id === organization._id ? organization : org)) ?? null,
      );
      setEditingOrganizationIndex(null);
  ...
  const handleOrganizationCreate = useCallback(async (organization: BaseOrganization) => {
    try {
      const newOrganization = await organizationService.createOrganization(organization);
      setOrganizations((prevOrganizations) => prevOrganizations?.concat(newOrganization) ?? null);
      setIsCreating(false);
