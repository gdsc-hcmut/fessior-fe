...
import { removeDuplicatedString } from '../../src/utils/common';
...
export default function DomainList({ domains, onDomainsChange }: DomainListProps) {
  const [isAdding, setIsAdding] = useState(false);
  const [domainInput, setDomainInput] = useState('');

  const handleAddDomain = () => {
    if (domainInput) {
      const newDomains = removeDuplicatedString(domainInput.split(' ')).filter(
    ...
    <div>
      {isAdding ? (
        <div className='mb-3 flex flex-col'>
          ...
          <p className='text-base italic text-primary'>Add multiple domains by separating them with a space.</p>
        </div>
      ) : (
        <Button
          className='mb-3 mt-1'
        ...
        {domains.map((domain) => (
          <DomainItem
            ...
            onDelete={(deletingDomain: string) => {
              onDomainsChange(domains.filter((currentDomain) => currentDomain !== deletingDomain));
            }}
            domain={domain}
          />
        ))}
